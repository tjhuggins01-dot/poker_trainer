...
export const policyKeyFromContext = (context: DrillContext): string => {
  ...
  if (context.nodeType === 'threeBet' && context.villainPos) {
    return makeThreeBetKey(
      context.heroPos as ThreeBetHeroPosition,
      context.villainPos,
      context.format,
      context.effectiveStackBb,
    );
...
export const policyKeyFromSituation = (
  situation: Situation,
  format: DrillFormat,
...
export const resolvePolicyRecord = (
  ...
  return {
    record: data.situations[legacyKey],
    key: data.situations[legacyKey] ? legacyKey : key,
  };
};
