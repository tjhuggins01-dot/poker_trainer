...
import { makeFacingOpenKey, makeRfiKey, makeThreeBetKey } from '../storage/keys';

const hasNoOverlap = (a: string[], b: string[]) => !a.some((hand) => b.includes(hand));

export type PresetApplyMode = 'rfi' | 'facing_open' | 'three_bet';

export type PresetSpot = {
  rfiPosition?: RfiPosition;
  heroPos?: FacingOpenHeroPosition | ThreeBetHeroPosition;
  villainPos?: Position;
};
...
export const applyPresetToAllRanges = (data: AppData, presetId: PresetId, context: DrillContext): AppData => {
  ...
  RFI_POSITIONS.forEach((pos) => {
    ...
    if (parsedRaise.ok && next.situations[key]) {
      (next.situations[key].policy as any).raise = parsedRaise.hands;
    }
  });
  ...
  if (parsedSbLimp.ok && next.situations[sbKey]) {
    (next.situations[sbKey].policy as any).limp = parsedSbLimp.hands;
  }
...
export const applyPresetToSpot = (
  ...
  if (mode === 'rfi' && spot.rfiPosition) {
    ...
    if (spot.rfiPosition === 'SB') {
      ...
      if (parsedLimp.ok && next.situations[key]) {
        (next.situations[key].policy as any).limp = parsedLimp.hands;
      }
    }
    return next;
  }
  ...
  if (mode === 'facing_open' && spot.heroPos && spot.villainPos) {
    const key = makeFacingOpenKey(
      spot.heroPos as FacingOpenHeroPosition,
      spot.villainPos,
      context.format,
  ...
  if (mode === 'three_bet' && spot.heroPos && spot.villainPos) {
    const bundle = getStackDataBundle(context.format, context.effectiveStackBb);
    const key = makeThreeBetKey(
      spot.heroPos as ThreeBetHeroPosition,
      spot.villainPos,
      context.format,
      context.effectiveStackBb,
    );
