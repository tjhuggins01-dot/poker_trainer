...
const makeRfiKey = (heroPos: RfiPosition, format: DrillFormat = 'cash9max', stack: EffectiveStackBb = 100): string => `RFI_${format}_${stack}BB_${heroPos}`;
const makeFacingOpenKey = (
  heroPos: FacingOpenHeroPosition,
  villainPos: Position,
  format: DrillFormat = 'cash9max',
...
const makeLimpIsoKey = (format: DrillFormat = 'cash9max', stack: EffectiveStackBb = 100): string =>
  `LIMP_ISO_${format}_${stack}BB_BB_VS_SB`;
const makeVsIsoKey = (format: DrillFormat = 'cash9max', stack: EffectiveStackBb = 100): string =>
